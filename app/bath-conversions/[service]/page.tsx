...
const hub = hubBySlug.get('bath-conversions')!;
...
export async function generateMetadata(
  ...
  return serviceMetadata(service, `/bath-conversions/${service}/`);
}

export default async function Page({ params }: { params: Promise<{ service: string }> }) {
  const { service } = await params;
  if (!hub.children.some((c) => c.slug === service)) notFound();
  return <ServicePage slug={service} path={`/bath-conversions/${service}/`} hub={hub} />;
}
